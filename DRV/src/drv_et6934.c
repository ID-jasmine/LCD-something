...
static void DRC_SetPixel(uint16_t point_id, bool state) {
    if (point_id < 1 || point_id > 384) return;
...
// 转速条映射 (0~39 共40个点)
static const uint16_t MAP_RPM[40] = {
    48, 45, 41, 44, 43, 42, 90, 91, 92, 89, 93, 96, 94, 95, 103, 102, 104, 101, 97, 100, 
    99, 98, 209, 216, 215, 214, 210, 211, 213, 212, 204, 205, 203, 202, 206, 207, 208, 201, 241, 248
};
...
// ---------------- 油量和电压映射 (0~4 级) ----------------
static const uint16_t MAP_FUEL[5][2] = {
    {85, 88}, {50, 51}, {49, 52}, {53, 56}, {54, 55}
};
...
static const uint16_t canvas_points[] = {
	...
	46, 105, 106, 107, 108, 109, 110, 111, 112, 113, 114, 115, 116, 117, 118, 119, 120,
	47, 57, 58, 59, 60, 81, 83, 84, 82, 121, 122, 123, 124, 125, 126, 127, 128,
	137, 142, 144, 249, 250, 251, 252, 253, 254, 255, 256,
	138, 139, 140, 141, 186, 193, 194, 195, 196, 197, 198, 199, 200, 242, 243, 244, 245, 246, 247,
	257, 260, 263, 264, 265, 267, 269, 272, 278, 313, 314, 315, 316, 317, 319, 320, 353, 355, 356, 358, 360,
	266, 270, 271, 322, 326, 268, 321, 324, 327, 328, 330, 334, 331, 333,
};
...
void DRC_ET6934_SetCanvas(bool state) {
    DRC_SetPixelList(canvas_points, sizeof(canvas_points)/sizeof(canvas_points[0]), state);
}

// 渲染独立指示灯
void DRC_ET6934_SetIndicator(DRC_Indicator_t ind, bool state) {
    switch (ind) {
        case IND_WATER_TEMP:   DRC_SetPixel(3, state); DRC_SetPixel(4, state); break;
        case IND_FAULT:        DRC_SetPixel(5, state); break;
        case IND_START_STOP:   DRC_SetPixel(8, state); break;
        case IND_ENGINE_FAULT: DRC_SetPixel(25, state); DRC_SetPixel(28, state); break;
        case IND_TURN_LEFT:    DRC_SetPixel(26, state); DRC_SetPixel(27, state); break;
        case IND_HIGH_BEAM:    DRC_SetPixel(29, state); DRC_SetPixel(32, state); break;
        case IND_P_GEAR:       DRC_SetPixel(30, state); DRC_SetPixel(31, state); break;
        case IND_TCS:          DRC_SetPixel(129, state); DRC_SetPixel(136, state); break;
        case IND_ABS:          DRC_SetPixel(130, state); DRC_SetPixel(131, state); break;
        case IND_TURN_RIGHT:   DRC_SetPixel(132, state); DRC_SetPixel(133, state); break;
        case IND_BATTERY_ALARM:DRC_SetPixel(134, state); DRC_SetPixel(135, state); break;
        case IND_SPEED_KMH:    DRC_SetPixel(182, state); DRC_SetPixel(190, state); break;
        case IND_SPEED_MPH:    DRC_SetPixel(183, state); DRC_SetPixel(191, state); break;
        case IND_ODO_MODE:     DRC_SetPixel(259, state); break;
        case IND_TRIP_MODE:    DRC_SetPixel(261, state); DRC_SetPixel(262, state); break;
        case IND_CLOCK_MODE:   DRC_SetPixel(277, state); break;
        case IND_TRIP_UNIT_MILES: DRC_SetPixel(290, state); DRC_SetPixel(346, state); DRC_SetPixel(354, state); break;
        case IND_TRIP_UNIT_KM:    DRC_SetPixel(338, state); break;
        case IND_SEC_JUMP_0:   DRC_SetPixel(298, state); break;
        case IND_SEC_JUMP_1:   DRC_SetPixel(306, state); break;
        case IND_BTN_SET:      DRC_SetPixel(314, state); DRC_SetPixel(317, state); break;
        case IND_FUEL_WHITE:   DRC_SetPixel(86, state); break;
        case IND_FUEL_YELLOW:  DRC_SetPixel(87, state); break;
        default: break;
    }
}
...
void DRC_ET6934_SetTrip(uint32_t trip_val) {
    if (trip_val > 999999) trip_val = 999999;
...
void DRC_ET6934_SetClock(uint8_t hour, uint8_t minute) {
    ...
    for (uint8_t seg = 0; seg < 7; seg++) {
        DRC_SetPixel(MAP_TRIP_DIGITS[1][seg], (font_7seg[m_u] & (1 << seg)) ? true : false);
    }
    // 3. 分钟十位 (使用 INDEX 2)
    uint8_t m_t = minute / 10;
    for (uint8_t seg = 0; seg < 7; seg++) {
        DRC_SetPixel(MAP_TRIP_DIGITS[2][seg], (font_7seg[m_t] & (1 << seg)) ? true : false);
    }
    // 4. 小时个位 (使用 INDEX 3 - 冒号左侧)
    uint8_t h_u = hour % 10;
    for (uint8_t seg = 0; seg < 7; seg++) {
        DRC_SetPixel(MAP_TRIP_DIGITS[3][seg], (font_7seg[h_u] & (1 << seg)) ? true : false);
    }
    // 5. 小时十位 (使用 INDEX 4)
    uint8_t h_t = hour / 10;
    if (h_t > 0) {
        for (uint8_t seg = 0; seg < 7; seg++) {
            DRC_SetPixel(MAP_TRIP_DIGITS[4][seg], (font_7seg[h_t] & (1 << seg)) ? true : false);
        }
    }
