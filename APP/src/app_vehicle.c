...
void Save_Mileage_To_EEPROM(void) {
    ...
    if (eeprom_seq % 2 == 0) {
        EEPROM_WriteBuffer(ADDR_SLOT_A, buffer, 8);
    } else {
        EEPROM_WriteBuffer(ADDR_SLOT_B, buffer, 8);
    }
}
void Load_Mileage_From_EEPROM(void) {
    ...
    while (retry--) {
        validA = 0;
        validB = 0;
        EEPROM_ReadBuffer(ADDR_SLOT_A, bufA, 8);
        EEPROM_ReadBuffer(ADDR_SLOT_B, bufB, 8);
